...
:stopping
echo Waiting for sometime before checking status of VMware Horizon Connection Server Service
TIMEOUT /T 60
goto SERVICESTOPPED

:stopped1
echo Vmware Horizon Connection Server Service has stopped
net start wsbroker
goto end
...
Echo         * Updates only the JVM heap settings in registry.
Echo         * Restarts VMware Horizon Connection Server services automatically after updating the registry
